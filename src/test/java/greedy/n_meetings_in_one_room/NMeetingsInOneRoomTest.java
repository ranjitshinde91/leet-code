...
class NMeetingsInOneRoomTest {
    @ParameterizedTest
    @MethodSource("inputs")
    void test1(int[] start, int[] end, int expected) {
        var nMeetingsInOneRoom = new NMeetingsInOneRoom();

        int output = nMeetingsInOneRoom.maxMeetings(start, end, start.length);
    ...
    private static Stream<Arguments> inputs() {
        return Stream.of(
                ...
                Arguments.of(new int[]{1, 3, 0, 5, 8, 5}, new int[]{2, 4, 6, 7, 9, 9}, 4),
                Arguments.of(new int[]{10, 12, 20}, new int[]{20, 25, 30}, 1)
        );
    }
}